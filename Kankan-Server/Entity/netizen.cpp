...
nlohmann::json Netizen::publishManuscript(nlohmann::json js, std::string& manuscriptId, std::string& videoId)
{
    ManuscriptProxy manuscriptProxy(manuscriptId);
    _manuscripts.insert({manuscriptId, manuscriptProxy});
    nlohmann::json json = manuscriptProxy.addManuscript(js, videoId);
    return json;
}

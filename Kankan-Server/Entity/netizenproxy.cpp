...
nlohmann::json NetizenProxy::publishManuscript(nlohmann::json js, std::string &manuscriptId, std::string& videoId)
{
    if (m_netizen == nullptr)
        m_netizen = NetizenBroker::getInstance()->findNetizenById(m_id);
    nlohmann::json json = m_netizen->publishManuscript(js, manuscriptId, videoId);
    return json;
}
